 ...
 * @param file
 */
function saveTodosToFile(todos, file) {
...
function TodoCtrl() {
  ...
  this.archivedFile = path.join(this.rootDir, '.todo.archive.md');
}
...
TodoCtrl.prototype = {
  ...
  addTodo: function(thing) {
    ...
    fs.appendFileSync(this.unfinishedFile, todo.toLine());
    return todo;
  },
  ...
  _save: function(type) {
    if (type === 'all' || type === 'archived') {
      saveTodosToFile(this.archivedTodos, this.archivedFile);
    }
    if (type === 'all' || type === 'unfinished') {
      saveTodosToFile(this.unfinishedTodos, this.unfinishedFile);
    }
    return true;
  },
